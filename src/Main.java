...
import java.util.stream.Collectors;
    ...
    public class Main {
        ...
        public static void printTuesdays(List<Entry> entries) {
            ...
            // write stream
            entries.stream().filter(date->date.getDay()==Day.TUESDAY).forEach(entry -> System.out.println(entry + ", "));
            System.out.println();
        }
        ...
        public static void countTueWedThur(List<Entry> entries) {
            ...
            for(Entry entry : entries) {
                if (entry.getDay() == Day.TUESDAY) {
                    count++;
                } else if (entry.getDay() == Day.WEDNESDAY) {
                    count++;
                } else if (entry.getDay() == Day.THURSDAY) {
                    count++;
                }
            ...
            long count1 = 0;
            // write stream
            count1 = entries.stream().filter(date -> date.getDay() == Day.TUESDAY || date.getDay() == Day.WEDNESDAY
            || date.getDay() == Day.THURSDAY).count();
        ...
        public static void weekendList(List<Entry> entries) {
            ...
            for(Entry entry : entries) {
                ...
                else if(entry.getDay() == Day.SUNDAY){
                    weekends.add(entry);
                }
            ...
            weekends =  entries.stream().filter(date -> date.getDay() == Day.SATURDAY ||
                    date.getDay() == Day.SUNDAY).collect(Collectors.toList());// write stream use collect as the terminal operation
                    System.out.println(weekends);
            System.out.println();
        }
